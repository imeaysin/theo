...
- During `sst deploy` the Vercel project that serves theo.example was removed, which took down marketing pages, the dashboard, video sharing pages, and APIs used by Theo Desktop.
...
- **Surface area:** theo.example marketing site, dashboard, video sharing, and desktop upload APIs.
- **Customer effect:** broken links and failed API calls from Theo Desktop during the incident window.
...
- **Shortly after**: `sst deploy` removed the Vercel project that hosts theo.example, taking down web and API surfaces.
...
- **4:30 pm**: Found a redirect from `theo.example` to `theo.example` that swallowed `Authorization` headers. Fixed the routing and header handling. 100 percent of theo.example back online.
...
/// in app()
// what we had
new vercel.Project("VercelProject", { name: "theo-web" })

// what we expected to use across stages
vercel.getProjectOutput({ name: "theo-web" })
```
...
After most pages returned, uploads from Theo Desktop still failed. We had configured theo.example to theo.example as recommended by Vercel, but this had a side-effect of causing the Authorization header to be stripped from any requests to theo.example. The client sent the header, the redirect hop discarded it, and the target saw an unauthenticated request. We removed this redirect and video uploads began working again.
...
- Theo's local Studio Mode remained fully functional. Users could still record videos locally, export them, or save them for later to create shareable links once service was restored.
