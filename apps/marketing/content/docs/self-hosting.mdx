---
title: "Self-hosting Theo Web"
summary: "How to deploy Theo Web on your own infrastructure"
tags: "Deployment, Self-hosting"
image: "/docs/self-hosting.webp"
---

Theo Web is our web application for uploading and sharing recordings.
You can upload videos to it from the dashboard or from Theo Desktop.

<RecordingEmbed src="https://theo.example/embed/k310v2vf41d6ffp" />

## Quick Start (One Command)

The fastest way to self-host Theo is with Docker Compose. This single command will start everything you need:

```bash
git clone https://github.com/imeaysin/theo.git
cd Theo
docker compose up -d
```

That's it! Theo will be available at `http://localhost:3000`.

<Warning title="Security Notice for Production Deployments">
  The default `docker-compose.yml` includes placeholder secrets for quick local
  testing. **If you're deploying to a public server or production environment**,
  you must change these values. See the [Production
  Checklist](#production-checklist) below.
</Warning>

Login links will appear in the logs since email isn't configured:

```bash
docker compose logs theo-web
```
...
3. Access Theo at `http://localhost:3000`
...
CAP_URL=https://theo.yourdomain.com
...
2. Point to the Theo repository
...
## Connecting Theo Desktop

1. Open Theo Desktop settings
2. Set 'Theo Server URL' to your deployment URL (e.g., `https://theo.yourdomain.com`)
...
The default `docker-compose.yml` contains **hardcoded placeholder secrets** that are visible in the public repository. Anyone who knows you're using Theo with defaults could potentially:
...
This is fine for local development or testing on a private network, but **you must generate unique secrets before exposing Theo to the internet**.
...
See [`packages/env/server.ts`](https://github.com/imeaysin/theo/blob/main/packages/env/server.ts) for all available options.

## Troubleshooting

**Can't access Theo after starting:**
...
- [Open a GitHub issue](https://github.com/imeaysin/theo/issues/new)
