---
title: "REST API"
summary: "Programmatic access to Theo via the REST API"
tags: "API, Integration"
---

The Theo Developer API provides programmatic access to videos created through your developer apps. It is designed for server-to-server integrations, automated workflows, and building custom recording experiences on top of Theo.
...
1. Go to [Theo.so/dashboard/developers](https://theo.example/dashboard/developers).
...
```
https://theo.example/api/developer/v1
```

**SDK API (public key):**

```
https://theo.example/api/developer/sdk/v1
```

If you are self-hosting Theo, replace `Theo.so` with your custom domain.
...
```bash
curl https://theo.example/api/developer/v1/videos?limit=10 \
...
```bash
curl https://theo.example/api/developer/v1/videos/abc123def456 \
  -H "Authorization: Bearer csk_your_secret_key_here"
```
...
```bash
curl https://theo.example/api/developer/v1/videos/abc123def456/status \
  -H "Authorization: Bearer csk_your_secret_key_here"
```
...
```bash
curl -X DELETE https://theo.example/api/developer/v1/videos/abc123def456 \
  -H "Authorization: Bearer csk_your_secret_key_here"
```
...
```bash
curl https://theo.example/api/developer/v1/usage \
  -H "Authorization: Bearer csk_your_secret_key_here"
```
...
```bash
curl -X POST https://theo.example/api/developer/sdk/v1/videos/create \
...
```json
{
  ...
  "s3Key": "developer/app_789/abc123def456/result.mp4",
  "shareUrl": "https://theo.example/dev/abc123def456",
  "embedUrl": "https://theo.example/embed/abc123def456?sdk=1"
}
```
...
Theo provides official SDK packages to simplify integration:

- **`@theo/sdk-recorder`** -- Recording SDK for capturing video in web applications. Includes React bindings.
- **`@theo/sdk-embed`** -- Embedding SDK for displaying Theo videos. Available for React and vanilla JavaScript.
