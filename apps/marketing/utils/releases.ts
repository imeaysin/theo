...
function extractVersionFromTag(tagName: string): string {
  return tagName.replace(/^theo-v/, "").replace(/^v/, "")
}

export async function getGitHubReleases(): Promise<Release[]> {
  const response = await fetch(
    "https://api.github.com/repos/imeaysin/theo/releases?per_page=100",
    {
      headers: {
  ...
  return data
    ...
    .filter((release) => release.tag_name.startsWith("theo-v"))
    .map((release) => ({
